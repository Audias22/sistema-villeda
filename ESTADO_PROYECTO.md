...
| Fase 5 | Funcionalidades nativas (cámara, notificaciones, biometría) | 🔄 En progreso — nombre/ícono/splash, escaneo con cámara, **build de APK con EAS** y **desbloqueo biométrico** completados (6 de septiembre de 2026, ver las secciones de abajo); solo quedan pendientes las notificaciones push |
...
**✅ RIESGO CONFIRMADO Y CORREGIDO — el logo rompía la generación del PDF en release (15 de septiembre de 2026):** la hipótesis anotada el 6 de septiembre, cuando se construyó el primer APK, **resultó correcta**. Se deja el razonamiento original acá abajo porque predijo el fallo antes de verlo, y porque la causa real es una variante más específica de lo previsto.

**La causa exacta.** `obtenerLogoBase64()` hacía `Image.resolveAssetSource(require('../assets/logo-villeda.jpg'))` y le pasaba el `asset.uri` a `File.downloadFileAsync()`. `resolveAssetSource()` **no devuelve lo mismo según el tipo de build**: en debug devuelve una URL `http://` al servidor de Metro, y en un build de release devuelve **un nombre aplanado de recurso drawable** — algo como `logo_villeda`, sin extensión, sin ruta y sin esquema — para que el cargador nativo lo saque del APK. O sea que no era "un URI no descargable por HTTP" como se anticipó: **no es un URI en absoluto**. `File.downloadFileAsync()` espera una URL, recibe ese nombre, no tiene nada que descargar y lanza. El error subía al `catch` de `generarPdf()` y salía el Alert genérico. Documentado en React Native, issues #24963 y #18216.

Síntoma en el APK: "Generar PDF" siempre fallaba con *"No pudimos generar el PDF. Intenta de nuevo."* **Sobrevivió tres builds sin diagnóstico** por la razón de la sección siguiente.

**⚠️ Esto NO significa que el logo esté roto en toda la app — distinción importante.** `AppHeader.js` y `PerfilScreen.js` usan el mismo `.jpg` con `<Image source={require('../assets/logo-villeda.jpg')} />` y **funcionan perfectamente en release**: ahí el componente nativo `Image` resuelve el asset por su cuenta, que es exactamente para lo que sirve `resolveAssetSource()` internamente. **El problema era sacar el URI crudo para pasárselo a otra API.** Esos dos usos son correctos y no se tocaron. Quien lea "el logo falla en release" sin esta aclaración va a ir a arreglar el header, que no tiene nada malo.

**⚠️ Lo que NO era el problema — descartado con evidencia, no revisar de nuevo.** Hubo un solo bug, no varios, y estos tres sospechosos quedaron limpios:

- **`Print.printToFileAsync` con base64 embebido funciona bien en release.** Lo prueba `EscanearDocumentoScreen.js`, que arma un PDF con exactamente la misma técnica y **venía funcionando en el APK desde el primer build**. **La única diferencia es el origen de las imágenes: las del escáner salen de `expo-image-manipulator`, o sea archivos reales en disco con una ruta `file://` de verdad, no assets empaquetados en el APK.** Por eso nunca pasan por `resolveAssetSource()` y nunca tocaron el bug. Este dato es el que evita que alguien en el futuro, viendo "el PDF falla en release", vaya a sospechar de `expo-print`.
- **`expo-file-system` y su API nueva de SDK 54 no tienen nada que ver.** El import es `from 'expo-file-system'` con `~19.0.23` instalada, y todo lo que se usa existe ahí: `Paths.cache`, `Paths.document`, `File.downloadFileAsync`, `.base64()`, `.move()`, `.delete()`, `.exists`. No hay mezcla con el subpath `legacy`. `File` y `Paths` siguen usándose en `generarPdf()` para mover el PDF terminado a `Paths.document`, sin cambios.
- **No hacen falta permisos de almacenamiento.** `Paths.document` es almacenamiento privado de la app en Android.

**El arreglo: la constante base64 incrustada** (`app-movil/src/assets/logoBase64.js`, nuevo). El JPG se lee una vez en tiempo de desarrollo y se guarda como cadena base64 en un `.js`; `obtenerLogoBase64()` ahora solo devuelve esa constante. Se va `resolveAssetSource()`, se va `downloadFileAsync()`, se va el archivo temporal en caché. El `.jpg` original **se conserva** porque AppHeader y PerfilScreen lo siguen usando y porque es la fuente para regenerar la constante (el comando está en el comentario de cabecera del archivo).

**Por qué base64 incrustado y no `expo-asset`, que es el camino canónico.** `Asset.fromModule()` + `await asset.downloadAsync()` + `asset.localUri` —el arreglo anotado el 6 de septiembre— **es la solución más correcta y sigue siéndolo**: mantiene un solo origen del logo y no infla el bundle. Se descartó por calendario, no por técnica. Dos razones: (1) no hay forma de verificarlo sin construir un APK, y si fallara costaría otro build completo con la defensa de tesis encima; (2) `expo-asset` está anidado en `node_modules/expo/node_modules/expo-asset`, no en la raíz, así que habría que instalarlo, y la decisión del proyecto es no agregar dependencias nuevas a esta altura. **El base64 incrustado no tiene forma de fallar: no hay asset que resolver, ni descarga, ni I/O.** El costo es ~29 KB de cadena en el bundle y tener el logo duplicado en dos formas. Si algún día se rehace con `expo-asset`, esta nota explica qué se estaba comprando a cambio.

**Resguardo agregado:** si por lo que sea la constante no está disponible, `obtenerLogoBase64()` devuelve `null` y el HTML se arma **sin membrete** en vez de romperse. Un reporte sin logo es mucho mejor que ningún reporte. Con la constante incrustada esto es casi imposible, pero cuesta cuatro líneas y garantiza que esta función no pueda volver a dejar la pantalla inservible.

**🐛 Por qué el bug sobrevivió tres builds: los `catch` se tragaban el error.** Tanto `generarPdf()` como `compartirPdf()` capturaban `err` y **nunca lo usaban** — mostraban un Alert genérico y listo. Tres APKs de release fallando sin una sola pista de la causa. Corregido: ambos ahora hacen `console.error` con el error real antes del Alert, y **el Alert dice qué paso falló** en vez de "Intenta de nuevo". `generarPdf()` lleva una variable `paso` que avanza por las tres etapas (armar el contenido → crear el archivo PDF → guardar el PDF en el dispositivo), y el mensaje incluye `err.message`. En un APK de release la consola solo se ve con `adb logcat`, así que llevar el detalle al Alert es lo que vuelve diagnosticable un fallo en el teléfono de la oficina. **Esta es la parte del arreglo que más vale a futuro:** el bug del logo era de una línea, pero la ceguera era estructural.

*Nota de método, que se sostiene:* el 6 de septiembre se decidió **no tocar el código y medir antes que cambiar a ciegas**, porque el comportamiento dependía del runtime. Fue lo correcto — la hipótesis apuntaba al lugar exacto, pero el mecanismo real (nombre de recurso drawable, no URL no descargable) solo se supo al verlo fallar.
...
`expo-local-authentication` quedó instalado y con su plugin declarado aunque la biometría todavía no esté implementada: **el plugin es lo que inyecta los permisos `USE_BIOMETRIC` y `USE_FINGERPRINT` en el manifiesto de Android, y eso solo se resuelve al construir**. Incluirlo ahora evita tener que reconstruir el APK cuando se implemente la funcionalidad. El costo es unos pocos KB de código nativo que ninguna pantalla usa todavía. Quedó confirmado: `eas init` materializó en `app.json` un bloque `android.permissions` con `USE_BIOMETRIC`, `USE_FINGERPRINT` y `CAMERA`, o sea que el APK ya trae declarados los permisos de biometría.

**Resultado del primer build:** proyecto `@audias22/oficina-villeda`, id `e50068cc-c4ef-4b33-883b-d1390f82c766`. Build `f8eeca16-d5dc-45c6-a3f2-cc04e60ef5e5`, perfil `preview`, SDK 54.0.0, versión 1.0.0, `versionCode` 2, commit `1cd446a`. Tardó **9 minutos y 26 segundos**. El APK queda en la URL de artefacto que imprime EAS y se instala escaneando el QR o abriendo el enlace del build desde el navegador del teléfono.

**⚠️ EL KEYSTORE — dato crítico que nadie recuerda hasta que lo necesita.** El keystore de firma **se generó en la nube y vive en la cuenta de EAS de `audias22`**, porque la máquina local no tiene `keytool` instalado. **Android exige que todas las actualizaciones de una app estén firmadas con el mismo keystore que la versión ya instalada.** Si ese keystore se pierde —cuenta borrada, acceso perdido— **no se va a poder actualizar nunca más la app instalada en los dispositivos de la oficina**: la única salida sería publicar una app nueva con otro `package`, desinstalar la vieja de cada teléfono y perder todo lo que la app tuviera guardado localmente. No hay forma de recuperarlo ni de firmar con otro. Se descarga con `npx eas-cli credentials` (plataforma Android → perfil, opción de descargar el keystore) y **conviene guardar esa copia fuera de la cuenta de EAS**, junto con la contraseña que el mismo comando muestra.

**Cómo reconstruir el APK más adelante:** `cd app-movil` y `npx eas-cli build --platform android --profile preview`. **Ojo con el modo no interactivo:** el CLI de EAS detecta por sí solo si no hay TTY y aborta antes de empezar cuando necesita preguntar algo — le pasó al primer intento, que murió con *"EAS project not configured. This command cannot configure it in non-interactive mode"*. Se resolvió corriendo antes `npx eas-cli init --account audias22 --non-interactive`, que es el comando que el propio error sugiere. Ahora que el proyecto ya está vinculado y el keystore existe, los builds siguientes no deberían necesitar ninguna confirmación — confirmado con el segundo build, que corrió sin una sola pregunta.

**Desbloqueo biométrico (6 de septiembre de 2026):** cierra el penúltimo pendiente de la Fase 5 móvil. El requerimiento a) del Capítulo IV declara autenticación biométrica, así que esto vuelve cierta esa afirmación. Archivos: `src/services/biometria.js` (nuevo), `src/screens/BloqueoScreen.js` (nuevo), y cambios en `storage.js`, `AuthContext.js`, `AuthNavigator.js`, `LoginScreen.js` y `PerfilScreen.js`. **`RootNavigator.js` no se tocó.**

**Qué es y qué no es.** La biometría **no reemplaza al login con usuario y contraseña**: es un atajo para reabrir la app con una sesión que ya existe. La primera vez siempre se entra con credenciales. Es **una comodidad con una capa de protección, no el mecanismo de seguridad principal** — el backend sigue exigiendo su propio token JWT.

**El gate: por qué NO es un modal.** `AuthContext` expone un estado nuevo, `desbloqueado`, y redefine el derivado como **`isAuthenticated: !!token && desbloqueado`**. Con eso `RootNavigator` sigue siendo la elección binaria de siempre y **`AppNavigator` simplemente nunca se monta** mientras la sesión esté bloqueada. La alternativa tentadora —montar la app y taparla con una pantalla de bloqueo por encima— **es insegura**: si `AppNavigator` llegó a montarse, el contenido real ya está en memoria y renderizado detrás, sus pantallas ya dispararon sus peticiones al backend, y basta un fallo de la capa superior para dejarlo a la vista. Manteniendo `isAuthenticated` en `false` no hay nada debajo que tapar. La elección entre pantalla de bloqueo y login vive en `AuthNavigator`, mediante un `bloqueado` que también expone el contexto.

**`desbloqueado` arranca en `true` y solo baja a `false` si hay motivo**, y esa orientación del valor por defecto es deliberada: si algo falla al detectar la biometría, la app se comporta como si no existiera y el usuario entra. Al revés —arrancar en `false` y subir al desbloquear— cualquier fallo dejaría al usuario encerrado fuera de su propia app. El gate exige **tres** condiciones: sesión guardada, preferencia activada, y un dispositivo que **hoy** pueda autenticar de verdad.

**Las cuatro decisiones, con su razón:**

1. **Token vencido tras la huella: se acepta, y está comentado en el código como comportamiento esperado.** La biometría protege el acceso a la app; el token del backend dura 15 minutos y es otra cosa. Si el usuario abre la app pasados esos 15 minutos, la huella lo deja entrar y la primera petición devuelve 401 con el aviso de sesión expirada de siempre. Con tokens tan cortos **este va a ser el caso habitual, no la excepción**. Se decidió no validar el token antes de celebrar el desbloqueo porque eso solo cambiaría el momento del aviso, no lo evitaría, a cambio de una petición extra y una rama de error más. **No hay que "arreglarlo"** haciendo que la biometría pida un token nuevo o guardando la contraseña: eso convertiría el desbloqueo local en re-autenticación contra el servidor y crearía la ilusión de una sesión más larga de la que existe.

2. **La preferencia sobrevive a `signOut()`.** Es una preferencia del dispositivo, no de la sesión: quien habilitó el desbloqueo en su teléfono espera que su teléfono siga siendo su teléfono después de cerrar sesión. `clearAll()` borra solo token y usuario, con un comentario explicando por qué la preferencia queda afuera —es una función de una línea que cualquiera ampliaría sin pensarlo—. Que quede una preferencia activa sin sesión es inofensivo: el gate exige **además** token guardado.

3. **Pregunta única tras el primer login MÁS interruptor en Perfil.** Sin el interruptor, un "Ahora no" accidental sería irreversible y el usuario quedaría sin la función para siempre. Se guardan **dos claves** en secure-store, `villeda_biometria_activa` y `villeda_biometria_preguntada`, y no una: responden preguntas distintas, y con una sola no se podría distinguir "dijo que no" de "todavía no se le preguntó", así que el ofrecimiento reaparecería en cada login. La fila del interruptor **no se muestra** si el dispositivo no soporta biometría.

4. **El gate corre solo al arranque en frío. Se evaluó `AppState` y SE DESCARTÓ**, con el motivo comentado en el código para que nadie lo agregue después creyendo que es una mejora obvia. Esta app manda al usuario afuera de sí misma por diseño y todo el tiempo: `expo-sharing` para mandar el PDF por WhatsApp, `expo-document-picker` para elegir un archivo, la cámara para escanear — y **en Android el selector de documentos y la hoja de compartir cuentan como segundo plano**. Con el gate en `AppState`, tocar "Compartir", elegir WhatsApp y volver pediría la huella otra vez con el PDF ya generado esperando; repetido decenas de veces por día, el resultado previsible es que el usuario apague la función. La ganancia sería chica porque el token expira solo a los 15 minutos, sobre un teléfono que además ya está detrás del bloqueo de pantalla de Android. **El costo de equivocarse es asimétrico:** pedir de más nos deja sin función, pedir de menos deja un riesgo residual que el backend ya acota. Si alguna vez hiciera falta, el punto medio es un umbral de tiempo (2 o 5 minutos), no un gate en cada vuelta.

**Casos límite resueltos.** *Cancelación*: no se relanza el prompt solo —reintentar sobre una cancelación deliberada dejaría al usuario sin salida— y quedan visibles "Usar {biometría}" y "Entrar con contraseña". *Lockout*: quien cuenta los intentos es Android, no la app; cuando devuelve `lockout` el sensor queda inhabilitado unos 30 segundos, así que **se oculta el botón de reintentar** (un botón que no puede funcionar es peor que ningún botón) y la única salida ofrecida es la contraseña. *Huellas borradas del sistema después de habilitar*: se comprueba `isEnrolledAsync()` **antes** de mostrar el prompt, y si ya no hay biometría registrada el gate no se activa, se apaga la preferencia y se entra normal; como segunda red, si el prompt devolviera `not_enrolled` o `not_available` se hace lo mismo. **El principio es "ante la duda, la app se abre"**: una app que no deja entrar a su dueño legítimo es peor falla que una que pide la huella de menos.

`etiquetaBiometria()` usa `supportedAuthenticationTypesAsync()` para que los textos digan "huella" o "reconocimiento facial" según lo que el dispositivo realmente haga, en vez de prometer huella en un teléfono que no la tiene. **"Entrar con contraseña" hace `signOut()` completo**: un token que nadie puede alcanzar no es una credencial guardada, es basura.

**Tres fallos encontrados en la revisión previa al build**, hecha a propósito porque el ciclo de prueba es de ~10 minutos por intento y no hay recarga en caliente:

- **`signOut()` sin `try/catch` podía encerrar al usuario.** Era el peor caso posible: `signOut()` es la **única** salida de la pantalla de bloqueo, y llamaba `await clearAll()` sin protección. Si `SecureStore.deleteItemAsync` fallaba, la excepción cortaba antes de `setDesbloqueado(true)` y la pantalla quedaba sin ningún botón que funcionara. Corregido con `try/catch/finally`: el borrado puede fallar, la limpieza de estado corre siempre. Es preferible un token huérfano —que expira solo— que alguien afuera de su propia app.
- **Efecto de montaje re-ejecutándose en cada render.** El efecto de `BloqueoScreen` dependía de `intentar`, que depende de `desbloquear`, que cambia de identidad en cada render de `AuthProvider`. El `ref` protegía el prompt de relanzarse pero `etiquetaBiometria()` se llamaba de más. Resuelto guardando `intentar` en un ref y dejando el efecto con dependencias vacías.
- El tercero era preexistente y va documentado aparte, abajo.

**✅ Verificado en dispositivo real (6 de septiembre de 2026)**, con el APK `versionCode 3` contra el backend de producción:

- **El desbloqueo funciona.** Con sesión guardada y la preferencia activa, cerrar la app por completo y reabrirla muestra la pantalla de bloqueo y pide la huella; al ponerla, entra directo.
- **Cancelar el prompt deja las dos salidas visibles**, que era el caso crítico de la revisión previa. "Entrar con contraseña" lleva al login y "Usar huella" relanza el prompt correctamente.
- **El interruptor de Perfil** enciende y apaga la preferencia como corresponde.
- **El lockout NO se llegó a reproducir, y el motivo es informativo:** ante varias huellas incorrectas, **Android ofrece el PIN de desbloqueo del dispositivo como alternativa ANTES de inhabilitar el sensor**. Al ingresarlo, `authenticateAsync()` devuelve éxito y la app entra. O sea que **en la práctica el usuario tiene una salida más de la que preveíamos, provista por el propio sistema operativo** — coherente con haber dejado `disableDeviceFallback: false`. El código que oculta el botón de reintentar ante `'lockout'` **se conserva** por si algún dispositivo o configuración sí llega a ese estado.

**⚠️ FALLO MENOR PENDIENTE — la pregunta única puede perderse (detectado el 6 de septiembre de 2026):** en `ofrecerBiometria()` de `LoginScreen.js`, **`saveBiometriaPreguntada()` se llama ANTES de mostrar el diálogo**. Si el usuario cierra sesión muy rápido tras el login, o el diálogo no llega a verse por cualquier motivo, la marca queda guardada igual y **el ofrecimiento no vuelve a aparecer nunca**. Ocurrió durante la verificación.

No bloquea nada: **el interruptor de `PerfilScreen` cubre el caso**, así que quien nunca vio la pregunta puede activar la biometría desde ahí — es exactamente el escenario para el que se pidió el interruptor. Por eso no se corrigió de inmediato: el ciclo de prueba son ~10 minutos de build y no justifica uno solo para esto.

**Arreglo cuando toque reconstruir el APK por otro motivo:** mover `saveBiometriaPreguntada()` **a dentro de los dos `onPress` del `Alert`**, para que la marca se guarde solo cuando el usuario efectivamente respondió algo.

**🐛 Bug PREEXISTENTE encontrado y corregido — no es parte de la biometría (6 de septiembre de 2026):** `cargarSesionGuardada()` en `AuthContext.js` llamaba `getUser()`, que hace `JSON.parse(raw)` sobre lo que haya en secure-store. **Con un valor corrupto eso lanza, `setIsLoading(false)` nunca se ejecuta, y la app queda colgada para siempre en la pantalla "Cargando…"** — sin login, sin ninguna salida, salvo desinstalar y reinstalar perdiendo la sesión. La app venía con esa fragilidad **desde antes de este trabajo**; se encontró al revisar el arranque para meter el gate biométrico y se corrigió en la misma tanda por estar tocando exactamente ese código. Fix: todo el cuerpo del cargador va en `try/catch` con `setIsLoading(false)` en un `finally`, así el arranque siempre termina. Por la misma razón, las cuatro funciones nuevas de preferencia en `storage.js` tampoco propagan excepciones.
- **Rediseño del PDF:** logo del despacho (`assets/logo-villeda.jpg`) embebido en el encabezado del HTML como base64, ya que `Print.printToFileAsync` no puede resolver un `require()` de React Native. *(El mecanismo de lectura descrito originalmente acá —`File.downloadFileAsync` sobre el asset resuelto vía `Image.resolveAssetSource`— **es justamente el que rompía el PDF en release**; desde el 15 de septiembre de 2026 el base64 viene de una constante incrustada en `src/assets/logoBase64.js`. Ver "RIESGO CONFIRMADO Y CORREGIDO" más arriba.)* CSS agregado (`@page { margin: 1cm }`, fuente base 11px, espaciados compactos entre `h1`/`h2`/`p`/`table`, `page-break-inside: avoid` en las tablas) para que el reporte quede en una sola página en vez de dos.
...
4. 🔄 Fase 5 móvil — completados: nombre/ícono/splash, escaneo con cámara multipágina, **build de APK con EAS** y **desbloqueo biométrico** (6 de septiembre de 2026). Pendiente: solo las notificaciones push (`expo-notifications`).
...
8. ✅ **Verificar el logo del PDF en el APK de release** (era el primer punto de la lista de verificación del APK, anotado el 6 de septiembre de 2026) — **cerrado el 15 de septiembre de 2026**: falló como se había previsto, la causa real es que `Image.resolveAssetSource()` devuelve un nombre de recurso drawable en release en vez de una URL, y `File.downloadFileAsync()` no puede consumirlo. Arreglado incrustando el logo como constante base64 en `src/assets/logoBase64.js`. De paso, los `catch` de `generarPdf()` y `compartirPdf()` dejaron de tragarse el error. Ver "RIESGO CONFIRMADO Y CORREGIDO" en la sección de la app móvil.
...
**Se elimina la bifurcación de extracción: Tesseract para TODOS los PDF en producción (11 de septiembre de 2026)**

El corpus de entrenamiento se extrajo forzando Tesseract (sección de arriba), pero **producción no lo forzaba**: `determinar_id_formato()` decidía, y si el PDF traía capa de texto la leía con pdfplumber. O sea que **el modelo recibía en inferencia un texto de calidad distinta al que había visto al entrenar.** Se eliminó esa bifurcación.

**⚠️ LA MEDICIÓN QUE LO MOTIVÓ — YA NO SE PUEDE REPRODUCIR.** Al cargar **180 expedientes reales del despacho a producción el 11 de septiembre de 2026**, la exactitud del clasificador se partió en dos según el método de extracción:

| método de extracción | aciertos | exactitud |
|---|---|---|
| **Tesseract** | **37 / 38** | **97.4 %** |
| **pdfplumber** | **126 / 142** | **88.7 %** |

**Casi nueve puntos de diferencia.** Y el 97.4 % de Tesseract **coincide con el 98.6 % de la validación cruzada** —que se hizo sobre texto de Tesseract—, así que el modelo no era el problema: **el problema era el texto que le llegaba**.

El detalle documento por documento está en **`Desktop\SEPARAR_PDF\cargados_con_bifurcacion.csv`** (fuera del repo). **Conservarlo importa: una vez forzado Tesseract dejaron de existir documentos extraídos con pdfplumber, así que esta comparación no se puede volver a medir.** Es evidencia del Capítulo V que solo existe en ese archivo.

**La causa** está descrita en la sección de arriba: la capa de texto que deja el escáner del despacho devuelve el contenido **desmenuzado carácter por carácter** — `c 1 ato - rc - e - de - o - c - tub - re` por "catorce de octubre". Para un clasificador entrenado sobre texto corrido, eso es otra distribución de entrada.

**Y el argumento decisivo no fue la clasificación sino la búsqueda.** `texto_completo` alimenta la **búsqueda por contenido**, que es el criterio 4 del sistema y una de las cinco formas de buscar que describe el Capítulo IV. En un texto fragmentado así, **la palabra "compraventa" no existe como tal**, de modo que buscarla no encuentra el documento. Con la mitad de los documentos en ese estado, no es una degradación estética: **inutiliza una funcionalidad central y contamina las mediciones de TBR del Capítulo V**, donde la búsqueda por contenido es uno de los criterios que se cronometran. O sea que la bifurcación degradaba la clasificación **y** la variable dependiente de toda la tesis.

**Qué cambió en el código.** `cargar_documento()` (`documentos/services.py`) y `_extraer_texto()` (`clasificacion/services.py`) llaman **siempre** a `procesar_archivo()`, el camino de Tesseract. Los dos caminos, no solo el del worker: el de la carga manual escribe el mismo `texto_completo` que alimenta la búsqueda.

**Costo aceptado: de ~1 s a ~30 s por documento.** En la carga manual (`POST /api/v1/documentos`) el usuario espera de verdad, porque ese endpoint procesa de forma síncrona; se verificó que el panel web muestra barra de progreso y el botón en "Procesando...", y la app móvil un `ActivityIndicator` con "Cargando y procesando...", así que la espera no parece un cuelgue. Los `timeout: 120000` de ambos clientes cubren los 30 s con margen. El endpoint del worker (`POST /clasificacion/trabajos`) no se ve afectado: responde 202 sin procesar y el OCR corre fuera del ciclo de petición.

**`id_formato` se sigue calculando, pero cambió de significado.** Se conserva por dos razones independientes: `documentos.id_formato` es **NOT NULL**, y es el dato que sostiene la medición de arriba. **Ahora describe QUÉ TRAÍA EL ARCHIVO DE ORIGEN, no cómo se extrajo el texto** — un documento con `id_formato = 2` (PDF digital) también pasó por Tesseract. Quien lea ese campo dentro de un año tiene que saberlo. Se verificó que ningún reporte, dashboard, filtro, consulta ni pantalla del panel o de la app lo consume: hoy se guarda y se expone en las respuestas JSON, y nadie lo lee.

**Cambio de comportamiento aceptado:** el chequeo de `resultado_ocr['exitoso']` ahora aplica a **todos** los documentos. Antes la rama de pdfplumber no lo verificaba, así que un PDF con capa de texto se cargaba aunque Tesseract no pudiera leer su imagen. Ahora ese caso devuelve error. **Es lo correcto**: si Tesseract no puede leer un documento, el sistema tiene que decirlo en vez de guardar en silencio un texto de otra procedencia. Fallar ruidosamente es mejor que la inconsistencia anterior.
